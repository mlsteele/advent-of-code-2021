...
def check_win(bolds):
    ...
    return set(winners)  # the board numbers
...
for pick in picks:
    prev_winners = check_win(bolds)
    shout(pick, boards, bolds)
    winners = check_win(bolds)
    if len(winners) == nboards:
        final_winner = list(winners - prev_winners)[0]
        answer = pick * score(final_winner, boards, bolds)
        print(answer)
        break
